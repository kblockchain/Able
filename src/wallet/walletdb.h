...
public:
    ...
    bool WriteZABLESeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZABLESeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZABLESeed_deprecated(uint256& seed);
    bool EraseZABLESeed();
    bool EraseZABLESeed_deprecated();

    bool WriteZABLECount(const uint32_t& nCount);
    bool ReadZABLECount(uint32_t& nCount);
