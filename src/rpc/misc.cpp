...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total able balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zABLEsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zABLE denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zABLE denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zABLE denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zABLE denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zABLE denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zABLE denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zABLE denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zABLE denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zABLE denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in able/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in able/kb\n"
    ...
    UniValue zableObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zableObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zableObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zABLEsupply", zableObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"ableaddress\"\n"
            "\nReturn information about the given able address.\n"

            "\nArguments:\n"
            "1. \"ableaddress\"     (string, required) The able address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ableaddress\", (string) The able address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ABLE address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are able addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) able address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"ableaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"ableaddress\"  (string, required) The able address to use for the signature.\n"
