...
#include "ui_zablecontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zableTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zableTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zABLE.";
            else if (!mint.isSeedCorrect)
                strReason = "The zABLE seed used to mint this zABLE is not the same as currently hold in the wallet";
            else
