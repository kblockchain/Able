...
// The zABLE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zABLE is a hash of the serial
...
{
    //Create an output returning the zABLE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZABLEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zABLE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zABLE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZABLEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zABLE output", __func__);
...
{
    CzABLETracker* zableTracker = pwallet->zableTracker.get();
    CMintMeta meta;
    if (!zableTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zableTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ABLE stake is the outpoint
