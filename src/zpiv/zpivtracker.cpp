// Copyright (c) 2018-2019 The ABLE developers
...
CzABLETracker::CzABLETracker(std::string strWalletFile)
{
...
CzABLETracker::~CzABLETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzABLETracker::Init()
{
...
bool CzABLETracker::Archive(CMintMeta& meta)
{
...
bool CzABLETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzABLETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzABLETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzABLETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzABLETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzABLETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzABLETracker::GetSerialHashes()
{
...
CAmount CzABLETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzABLETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzABLETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzABLETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzABLETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzABLETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzABLETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzABLETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzABLETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzABLETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzABLETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzABLEWallet* zABLEWallet)
{
    bool iszABLEWalletInitialized = (NULL != zABLEWallet);
    CMintMeta meta;
    ...
    if (! iszABLEWalletInitialized)
        zABLEWallet = new CzABLEWallet(strWalletFile);
    meta.isSeedCorrect = zABLEWallet->CheckSeed(dMint);
    if (! iszABLEWalletInitialized)
        delete zABLEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzABLETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzABLETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzABLETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzABLETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzABLETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzABLETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzABLEWallet* zABLEWallet = new CzABLEWallet(strWalletFile);
